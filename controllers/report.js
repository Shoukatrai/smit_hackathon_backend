...
`;
        ...
        const text = (await aiResult.response.text()).replace(/```json|```/g, '').trim();
        let data;
        console.log("text", text)
        try { data = JSON.parse(text); }
        catch { return res.status(500).json({ message: 'Error parsing AI response' }); }
        const savedObj = {
            reportName: reportName || file.originalname.split(".")[0],
            reportDate: new Date(),
            userId: req.user.id,
            family: family,
            filePath: fileUrl,
